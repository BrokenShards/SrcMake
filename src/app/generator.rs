...
pub fn generate_file(appdata: &AppData) -> SMResult<()>
{
	if !appdata.valid()
	{
		...
		ft.push( thread::spawn( move || {
				...
				.is_empty()
			{
				...
				let scriptdata = {
					...
					scriptpath += &ad.languages[ad.language].script_path;
				...
				match lua.context(|lua_ctx| {
					...
					if globals
						.set("SMSafeName", path_to_name(&ad.name, '_'))
						.is_err()
					{
			...
			content = replace_universal_macros(&ad, content, fileext);
			// Cleanup whitespace.
			...
			targetpath.push(&ad.directory);

			let dataname = ad.name.replace("\\", "/");
			...
			let fname = get_file_name(&ad.name, true);
			...
			if exists && ad.overwrite.is_none()
			{
				println!(
			...
			else if exists && ad.overwrite.is_some()
			{
				let ov = ad.overwrite.unwrap();
